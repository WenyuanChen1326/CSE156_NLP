...
def learn_trigram(data, verbose=True, laplace = 0.001):
    """Learns a unigram model from data.train.

    It also evaluates the model on data.dev and data.test, along with generating
    some sample sentences from the model.
    """
    from trigram_laplace import Unigram
    from trigram_laplace import Trigram
    unigram = Unigram()
    unigram.fit_corpus(data.train)
    trigram = Trigram(unigram, laplace=laplace)
    ...
    if verbose:
        ...
        from generator_tri import Sampler
...
if __name__ == "__main__":
    ...
    # This is for small set of corpus B
    #size = int(len((datas[1]).train)/3)
    # corpus_AB = (datas[0]).train + (datas[1]).train[:size]

    # The following two lines include full set of corpus B
    corpus_AB = (datas[0]).train + (datas[1]).train
    datas[0].train = corpus_AB
    data = datas[0]
    models[0] = learn_unigram(data)
    # For trigram with laplace smoothing. Can input different value for laplace value
   #  model = learn_trigram(data, laplace = 0.0001)
